class Ecran {
  ...
  Ecran (float L, float l, float h, float e, PImage texFace, PImage texDos, PImage texCote ) {
    this.L = L;
    this.l = l ;
    this.h = h ;
    this.e = e;
  ...
  PShape getEcran() {
    ...
    ecran.translate(0, random(0, l/4));
    s.addChild(ecran);
  ...
  PShape getSupport() {
    ...
    PShape support = (new Cube(texDosEcran)).getCube();



    PShape s = createShape(GROUP);

    base.scale(L/3, l/2.5, e/2);
    base.rotateX(PI/2);
    base.translate(0, L/2 , 2*e);
    s.addChild(base);
    
    support.scale(l/5, L/2, e);
    support.translate(0, L/4, 2*e);
    s.addChild(support);
